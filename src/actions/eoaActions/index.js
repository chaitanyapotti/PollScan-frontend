...
export const checkVbsMembership = (userAddress, vbsFile) => {
  ...
  return dispatch => {
    ...
    httpClient({
      method: "post",
      url: `${config.api_base_url}/eoa/check/vbsmembership?useraddress=${userAddress}&network=${network}`,
      data: form,
...
export const checkEntityMembership = (userAddress, entityAddress) => dispatch => {
  const network = "rinkeby";
  axios
    .get(`${config.api_base_url}/eoa/check/membership`, { params: { useraddress: userAddress, entityaddress: entityAddress, network } })
    .then(response => {
...
export const fetchUserPollActivity = (userAddress, pollAddress) => dispatch => {
  const network = "rinkeby";
  axios
    .get(`${config.api_base_url}/eoa/poll/activity`, { params: { useraddress: userAddress, polladdress: pollAddress, network } })
    .then(response => {
      if (response.data.message === constants.SUCCESS) {
        dispatch({
